export default function filamentGoogleMapsField({
    ...
    return {
        ...
        createMap: function () {
                ...
                if (drawingField) {
                    ...
                    google.maps.event.addListener(
                        ...
                        (event) => {
                            ...
                            // Tourne le mode de dessin à null après avoir dessiné une forme.
            ...
            //Ajout de l'écoute sur le champ des données de polygon
            this.$watch("drawingFieldState", () => {
                let geoJSON = getStateUsing(drawingField);

                if (!geoJSON) {
                    //Récupération des anciens paramètres de la carte
                    let centreCarte = this.map.getCenter();
                    let latitude = centreCarte.lat();
                    let longitude = centreCarte.lng();
                    let zoom = this.map.getZoom();

                    //Regénération de la carte
                    this.createMap()

                    //Ajout des anciens paramètres à la nouvelle carte
                    let nouvellePosition = new google.maps.LatLng(latitude, longitude);
                    this.map.setCenter(nouvellePosition);
        ...
        instanceOverlay: function (feature) {
            ...
            switch (feature.properties.type) {
                ...
                case google.maps.drawing.OverlayType.POLYGON:
                    ...
                    //A l'ajout d'un polygone, on désactive le mode de dessin
                    this.drawingManager.setDrawingMode(null);
                    this.drawingManager.setOptions({drawingControl: false});
                    break;
